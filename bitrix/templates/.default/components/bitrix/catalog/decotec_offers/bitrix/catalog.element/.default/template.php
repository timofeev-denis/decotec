...
<?php echo __FILE__; ?>
<?php
//var_dump( $arResult["ATT_MORE_PHOTO"] );
?>
...
<div class="catalog-element">
	...
	<?if(is_array($arResult[ "PROPERTIES" ]["ATT_MORE_PHOTO"]["VALUE"]) && count($arResult[ "PROPERTIES" ]["ATT_MORE_PHOTO"]["VALUE"]) > 0):?>
		ARRAY!
		<?foreach($arResult[ "PROPERTIES" ]["ATT_MORE_PHOTO"]["VALUE"] as $pic):?>
			<div style="float:left;"><img src="<?=CFile::GetPath($pic)?>" /></div>
		<?endforeach?>
	<?else:?>
		NO ARRAY :(
	<?endif;?>
	
	Страна: <?=$arResult["PROPERTIES"]["ATT_COUNTRY"]["VALUE"]?><br />
	Фирма: <?=$arResult["PROPERTIES"]["ATT_BRAND"]["VALUE"]?><br />
	Вид: <?=$arResult["PROPERTIES"]["ATT_TYPE"]["VALUE"]?><br />
	Тематика: <?=$arResult["PROPERTIES"]["ATT_SUBJECT"]["VALUE"]?><br />
	<?if(is_array($arResult["OFFERS"]) && !empty($arResult["OFFERS"])):?>
		<?foreach($arResult["OFFERS"] as $arOffer):?>
		
			<?php //print_r( $arOffer ); ?>
			<hr />
			<img border="0" src="<?=CFile::GetPath($arOffer["PREVIEW_PICTURE"])?>" width="<?=$arOffer["PREVIEW_PICTURE"]["WIDTH"]?>" height="<?=$arOffer["PREVIEW_PICTURE"]["HEIGHT"]?>" alt="<?=$arOffer["NAME"]?>" title="<?=$arOffer["NAME"]?>" id="image_<?=$arOffer["PREVIEW_PICTURE"]["ID"]?>" style="display:block;cursor:pointer;cursor: hand;" />
