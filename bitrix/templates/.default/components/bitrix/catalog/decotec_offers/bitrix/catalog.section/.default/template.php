...
<table cellpadding="0" cellspacing="0" border="0">
		...
		<td valign="top" width="<?=round(100/$arParams["LINE_ELEMENT_COUNT"])?>%" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
			...
			<?//print_r( $arElement );?>
			Страна: <?=$arElement["PROPERTIES"]["ATT_COUNTRY"]["VALUE"]?><br>
			Фирма: <?=$arElement["PROPERTIES"]["ATT_BRAND"]["VALUE"]?><br>
			Вид: <?=$arElement["PROPERTIES"]["ATT_TYPE"]["VALUE"]?><br>
			Тематика: <?=$arElement["PROPERTIES"]["ATT_SUBJECT"]["VALUE"]?><br>
			<?if(is_array($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):?>
				<?foreach($arElement["OFFERS"] as $arOffer):?>
					<?print( $arOffer[ "NAME" ] . "<br>\n" );?>
					<?//print_r( $arOffer );?>
