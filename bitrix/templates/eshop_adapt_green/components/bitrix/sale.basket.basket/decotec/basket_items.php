					...
					<tr id="<?=$arItem["ID"]?>">
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								<td class="itemphoto">
									<div class="bx_ordercart_photo_container">
										...
										?>
										<?
										$parentInfo = CCatalogSku::GetProductInfo( $arItem["PRODUCT_ID"] );
										$dataEx = CCatalogProduct::GetByIDEx( $parentInfo[ "ID" ] );
										?>
										<?if (strlen($dataEx["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$dataEx["DETAIL_PAGE_URL"] ?>"><?endif;?>
											<div class="bx_ordercart_photo" style="background-image:url('<?=$url?>')"></div>
										<?if (strlen($dataEx["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
									</div>
									<?
							...
							?>
								<td class="custom">
									<div class="centered">
										<table id="item_quantity" cellspacing="0" cellpadding="0" class="counter">
											<tr>
												<td>
													<?
							...
							?>
								<td class="custom">
									...
									if ($arHeader["id"] == "SUM"):
									?>
										<div id="sum_<?=$arItem["ID"]?>" class="center">
									<?
									endif;
