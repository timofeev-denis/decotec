...
class CSecurityXSSDetectVariables
{
	 ...
	 * @return string
	 */
	protected static function preparePatternValue($value)
	{
		// Makes no sense to look for the whole data, just select enough data to avoid false positive.
		// This is very similar to XSSAuditor
		$result = substr($value, 0, static::MAX_PATTERN_LENGTH);
		$result = preg_quote($result, static::PATTERN_DELIMITER);
		return $result;
	}
	...
	protected static function getPattern($pNeedle, $pIsQuotePattern = false)
	{
		...
		$needles = array_map(array('static', 'preparePatternValue'), $needles);
